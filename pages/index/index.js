...
Page({
  ...
  onShareAppMessage: function () {
    return {
      title: config.getWebsiteName+' 个人博客小程序',
            ...
            if (response.statusCode === 200) {
                ...
                self.setData({
                    ...
                    postsList: self.data.postsList.concat(response.data.map(function (item) {
                      ...
                      if (item.content.rendered.substr(0, 4) == '<img' || item.content.rendered.substr(0, 7) == '<p><img')
                      {
                        var strResult = item.content.rendered.match(/<img.*?(?: |\\t|\\r|\\n)?src=[\'"]?(.+?)[\'"]?(?:(?: |\\t|\\r|\\n)+.*?)?>/);
                        if (strResult != null) {
                          item.post_thumbnail_image = strResult[1];
                        }
                      }
